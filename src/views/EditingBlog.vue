<template>
    <div class="create-post" v-if="this.$store.state.superUser">
        <BlogCoverPreview v-show="this.$store.state.blogPhotoPreview"/>
        <Loading v-show="loading" />
        <div class="container">
            <div :class="{invisible: !error}" class="errorMsg">
                <p class="err-message"><span>Error:</span>{{ this.errorMsg}}</p>
            </div>
            <div class="blog-info">
...
export default {
    ...
    methods: {
        ...
        async updateBlog() {
            ...
            if (this.blogTitle.lenth !== 0 && this.blogHTML.lenth !== 0) {
                if (this.file) {
                    this.loading = true;
                    const storageRef = firebase.storage().ref();
...
.create-post {
    ...
    .blog-info {
        ...
        input {
           transition: all ease-in-out all;
           padding: 10px 4px;
           border: none;
